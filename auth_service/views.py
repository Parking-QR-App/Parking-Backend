...
class VerifyOTPView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # 🔹 Check if FCM token and device type were provided
            if fcm_token and device_type:
                existing_device = UserDevice.objects.filter(fcm_token=fcm_token).first()

                # 🔸 If FCM token exists for another user
                if existing_device and existing_device.user != user:
                    errors["fcm_token"] = "This FCM token is already registered to another user."
                    return Response(
                        {"message": "Failed to register device.", "errors": errors, "status": status.HTTP_400_BAD_REQUEST},
                        status=status.HTTP_400_BAD_REQUEST
                    )

                # 🔸 If FCM token exists but has a different device type, update it
                if existing_device and existing_device.device_type != device_type:
                    existing_device.device_type = device_type
                    existing_device.save()

                # 🔸 If FCM token is new, store it
                elif not existing_device:
                    UserDevice.objects.create(user=user, fcm_token=fcm_token, device_type=device_type)

            refresh = RefreshToken.for_user(user)

            # Fetch user's active QR code
            qr_info = None
